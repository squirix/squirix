...
{
    private static bool IsQuickMode =>
        string.Equals(System.Environment.GetEnvironmentVariable("SQUIRIX_BENCH_QUICK"), "1", StringComparison.Ordinal);

    /// <summary>Returns publish operations per invoke (default 2 000; quick 500).</summary>
    ...
    public static int ResolvePublishOperationsPerInvoke(int defaultCount = 2_000) =>
        IsQuickMode ? Math.Max(defaultCount / 4, 500) : defaultCount;
    ...
    public static int ResolveRollsPerInvoke(int defaultCount = 4) =>
        IsQuickMode ? Math.Max(defaultCount / 2, 2) : defaultCount;
    ...
    public static int ResolveRetentionCount(int defaultCount = 100_000) =>
        IsQuickMode ? Math.Max(defaultCount / 10, 10_000) : defaultCount;
}
