...
internal static class TestNodeCluster
{
    ...
    [SuppressMessage(
        ...
        "CA2000:Dispose objects before losing scope",
        Justification = "Ownership of the identity and data directory transfers to the returned cluster, which disposes them.")]
    private static ValueTask<TestCluster<ClusterStartOptions>> StartHeldAsync(
        int replicaCount,
        ClusterIdentity? identity,
        TempDirectory? dataDir,
        HeldPort[] held,
        ClusterNode[] topology,
        CancellationToken cancellationToken) => TestCluster<ClusterStartOptions>.Create(topology, identity: identity, dataDir: dataDir).StartAllAsync(
        nodeId => new ClusterStartOptions
        {
            ReplicaCount = replicaCount,
            DataDir = dataDir == null ? null : NodePathKit.Combine(dataDir, nodeId),
        },
        (started, total) =>
        {
