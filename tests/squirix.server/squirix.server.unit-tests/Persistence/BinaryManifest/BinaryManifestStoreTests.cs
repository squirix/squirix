...
/// <summary>Integration tests for the binary manifest store backend.</summary>
public sealed class BinaryManifestStoreTests : UnitTestBase, IAsyncLifetime
{
    ...
    /// <summary>Verifies the first write creates a binary pointer and numbered manifest file.</summary>
    [Fact]
    public async Task WriteAsyncCreatesBinaryCurrentPointerAndManifestFile()
    {
        var options = new PersistenceOptions { DataDir = Dir.Path, ManifestBackend = ManifestBackend.Binary };
        ...
        var manifest = BinaryManifestCodec.Decode(await File.ReadAllBytesAsync(manifestPath, DefaultCancellationToken));
    ...
    public async Task PublishBlockingIncrementsIndexWithoutDiskRead()
    {
        ...
        var currentPath = PathKit.Combine(Dir.Path, "man-current");
        Assert.Equal(2, BinaryManifestPointer.Read(await File.ReadAllBytesAsync(currentPath, DefaultCancellationToken)));
    }
    ...
    {
        _dir = new TempDirectory("binary-manifest");
        return ValueTask.CompletedTask;
    }
}
