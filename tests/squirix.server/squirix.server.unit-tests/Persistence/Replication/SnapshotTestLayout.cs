...
internal static class SnapshotTestLayout
{
    ...
    private const int PayloadLengthFileOffset = MagicByteCount + VersionByteCount;
    ...
    internal static void WriteCommitIndex(byte[] bytes, ulong value) => BinaryPrimitives.WriteUInt64LittleEndian(
        bytes.AsSpan(HeaderByteCount + CommitIndexPayloadOffset, FixedFieldByteCount),
        value);
}
