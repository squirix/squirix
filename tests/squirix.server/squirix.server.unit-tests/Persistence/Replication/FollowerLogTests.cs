...
public sealed class FollowerLogTests : ServerUnitTestBase
{
    ...
    /// <summary>The term at an index is read from the retained entry, zero at the log origin and past the last entry.</summary>
    /// <param name="cancellationToken">The test cancellation token.</param>
    [Test]
    public async Task TermAtReadsRetainedEntries(CancellationToken cancellationToken)
    {
        ...
        _ = await Assert.That(await log.GetTermAtAsync(3UL, cancellationToken)).IsEqualTo(0UL);
    }
