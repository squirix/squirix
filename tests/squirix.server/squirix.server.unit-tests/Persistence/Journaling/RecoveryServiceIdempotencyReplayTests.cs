...
public sealed class RecoveryServiceIdempotencyReplayTests : UnitTestBase
{
    ...
    [Fact]
    public async Task JournalReplayRestoresIdempotencyCreatedUtcFromUnixMs()
    {
        ...
        var exportedRecord = Assert.Single(idempotencyStore.ExportSnapshot(DateTime.UtcNow));
    ...
    {
        foreach (var record in JournalReadPath.ReadAll(dataDir, 1, CancellationToken.None))
        {
