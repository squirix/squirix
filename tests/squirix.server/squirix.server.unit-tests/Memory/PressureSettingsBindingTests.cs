...
public sealed class PressureSettingsBindingTests : ServerUnitTestBase
{
    ...
    public async Task DeserializeAndMergeIntoAppliesJsonOverrides()
    {
        ...
        var path = await WriteSettingsAsync("""{"MemoryPressure":{"maxEstimatedCacheBytes":4096,"highPressureThresholdPercent":70,"criticalPressureThresholdPercent":90}}""").ConfigureAwait(true);
        try
        {
            var (found, merged) = await PressureBootstrap.TryMergeFromSettingsFilePathAsync(path, baseline, DefaultCancellationToken).ConfigureAwait(true);
    ...
    public async Task DeserializeAndMergeKeepsBaselineForAbsentFields()
    {
        ...
        var path = await WriteSettingsAsync("""{"MemoryPressure":{"highPressureThresholdPercent":60}}""").ConfigureAwait(true);
        try
        {
            var (found, merged) = await PressureBootstrap.TryMergeFromSettingsFilePathAsync(path, baseline, DefaultCancellationToken).ConfigureAwait(true);
    ...
    private static async Task<string> WriteSettingsAsync(string json)
    {
        ...
        await File.WriteAllTextAsync(path, json, DefaultCancellationToken).ConfigureAwait(true);
        return path;
    }
}
