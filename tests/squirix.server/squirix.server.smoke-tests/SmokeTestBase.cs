...
public abstract class SmokeTestBase : IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var cluster = TestCluster<SmokeStartOptions>.Create(topology, (self, _, options, token) => StartClusterNodeAsync(self.Uri, peers, options, token), peers);

        try
        {
            for (var i = 0; i < topology.Length; i++)
                _ = await cluster.StartNodeAsync(topology[i].NodeId, factory?.Invoke(topology[i].NodeId), cancellationToken).ConfigureAwait(false);

            return cluster;
        }
        catch
        {
            // Release the held ports of nodes that never bound before stopping the nodes that did start.
            var started = cluster.StartedCount;
            await cluster.DisposeAsync().ConfigureAwait(false);

            for (var i = started; i < topology.Length; i++)
                ListenPortPool.SmokeTests.ReleasePort(topology[i].Uri.Port);

            throw;
        }
    }

    private async ValueTask<ITestNodeHost> StartClusterNodeAsync(Uri uri, ServerPeer[] peers, SmokeStartOptions? options = null, CancellationToken cancellationToken = default)
    {
