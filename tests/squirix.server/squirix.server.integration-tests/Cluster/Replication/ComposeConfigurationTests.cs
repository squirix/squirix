...
public sealed class ComposeConfigurationTests : NodeIntegrationTestBase
{
    ...
    public async Task ReleaseComposeMatchesRfThreeLayout(CancellationToken cancellationToken)
    {
        ...
        var releaseCompose = await AssertComposeFileAsync(root, "docker-compose.release.yml", "SQUIRIX_CLUSTER_MTLS_CERT_PFX_PASSWORD=${", cancellationToken);
        await AssertHomogeneousImageVersion(releaseCompose);
    ...
    {
        var root = RepositoryRootFinder.Find();
        _ = await AssertComposeFileAsync(root, "docker-compose.yml", "SQUIRIX_CLUSTER_MTLS_CERT_PFX_PASSWORD=dev-docker-mtls", cancellationToken);
    ...
    private static async Task<string> AssertComposeFileAsync(string root, string fileName, string passwordFragment, CancellationToken cancellationToken)
    {
        ...
        await AssertServiceBlock(compose, "node-a", "  node-b:", "5001:5000", "node-A.pfx", "nodea-data", passwordFragment);
        await AssertServiceBlock(compose, "node-b", "  node-c:", "5002:5000", "node-B.pfx", "nodeb-data", passwordFragment);
        await AssertServiceBlock(compose, "node-c", "\nvolumes:", "5003:5000", "node-C.pfx", "nodec-data", passwordFragment);
    ...
    private static async Task AssertContainsInBlock(string compose, int start, int end, string fragment, string service)
    {
    ...
    private static async Task AssertHomogeneousImageVersion(string compose)
    {
    ...
    private static async Task AssertServiceBlock(string compose, string service, string nextMarker, string port, string cert, string volume, string passwordFragment)
    {
        ...
        await AssertContainsInBlock(compose, start, end, port, service);
        await AssertContainsInBlock(compose, start, end, cert, service);
        await AssertContainsInBlock(compose, start, end, volume + ":/data", service);
        await AssertContainsInBlock(compose, start, end, "SQUIRIX_CLUSTER_MTLS_CA_PATH=/mtls/cluster-ca.crt", service);
        await AssertContainsInBlock(compose, start, end, passwordFragment, service);
        await AssertContainsInBlock(compose, start, end, "SQUIRIX_CLUSTER_MTLS_INTERNAL_PORT=5100", service);
        await AssertContainsInBlock(compose, start, end, "\"run\", \"--persist\", \"--data-dir\", \"/data\"", service);
    }
