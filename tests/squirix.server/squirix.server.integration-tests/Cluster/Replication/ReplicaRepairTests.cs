...
public sealed class ReplicaRepairTests : NodeIntegrationTestBase
{
    ...
    public async Task ReconcileNoOpAtTailSucceeds()
    {
        ...
        var mismatch = await log.ReconcileTailAsync(3UL, 2UL, 1UL, DefaultCancellationToken);

        Assert.False(mismatch.Success);
        Assert.Equal(FollowerLogRefusal.LogMismatch, mismatch.RefusalCode);
        Assert.False(mismatch.Quarantined);
    }
