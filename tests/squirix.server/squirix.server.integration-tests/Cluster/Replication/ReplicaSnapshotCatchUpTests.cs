...
public sealed class ReplicaSnapshotCatchUpTests : NodeIntegrationTestBase
{
    ...
    public async Task ConflictingTopologyQuarantinesReplica()
    {
        ...
        await using var follower = new FollowerLog(targetDir, GroupId, GroupComposition.Create(GroupId));
        await follower.OpenAsync(DefaultCancellationToken);
        var installed = await follower.InstallSnapshotAsync(SnapshotWith([7]), 1UL, DefaultCancellationToken);
    ...
    [Fact]
    public async Task StaleLeaderTermRefusedWithoutQuarantine()
    {
        using var targetDir = new TempDirectory("squirix-catch-up-stale-term");
        await using var follower = new FollowerLog(targetDir, GroupId, GroupComposition.Create(GroupId));
        await follower.OpenAsync(DefaultCancellationToken);
        Assert.True((await follower.AppendAsync(Append(1UL, 3UL, "one"), DefaultCancellationToken)).Success);
        ...
        Assert.False(eligibility.CanCountInWriteQuorum(1));
        Assert.Null(follower.SnapshotPath);
        Assert.Equal(3UL, (await follower.GetStatusAsync(DefaultCancellationToken)).CurrentTerm);
    }
