...
public sealed class TypedValueTests(SingleNodeFixture fixture) : TestBase(fixture)
{
    ...
    public async Task GetEntryReturnsTypedValueAndMetadata()
    {
        ...
        await cache.SetAsync("k", expected, Expiry.In(TimeSpan.FromMinutes(5)), DefaultCancellationToken);
    ...
    public async Task UpdatePreservesCustomRecordExpiry()
    {
        ...
        await cache.SetAsync("k", TypedValueFactory.CreateProfile("update"), Expiry.In(TimeSpan.FromMinutes(5)), DefaultCancellationToken);
