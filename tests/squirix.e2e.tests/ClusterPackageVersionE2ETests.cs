...
public sealed class ClusterPackageVersionE2ETests : EndToEndTestBase
{
    ...
    public async Task MismatchedPackageVersionFailsReadiness(CancellationToken cancellationToken)
    {
        ...
        var dirB = NodePathKit.Combine(dir.Path, "nodeB");
        ...
        _ = await cluster.StartNodeAsync("nodeA", new ClusterStartOptions { ReplicaCount = 2, DataDir = NodePathKit.Combine(dir.Path, "nodeA") }, cancellationToken);
