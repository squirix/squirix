...
internal static class CacheWireCodec
{
    internal static T? FromCacheValue<T>(CacheValue value, ISquirixSerializer serializer)
    {
        ...
        return value.KindCase switch
        {
            ...
            CacheValue.KindOneofCase.Payload when !value.Payload.IsEmpty => DecodePayload<T>(value.Payload.Span, serializer),
            _ => throw new InvalidDataException("Unsupported cache value kind."),
        };
    }

    internal static T? FromEntryWireValue<T>(CacheEntryWire entry, ISquirixSerializer serializer)
    {
        ...
        return DecodePayload<T>(entry.Payload.Span, serializer);
    }
    ...
    internal static CacheValue ToCacheValue<T>(T? value, ISquirixSerializer serializer)
    {
        ...
        return value switch
        {
            ...
            JsonElement json => ToCacheValueFromJsonElement(json, serializer),
            _ => new CacheValue { Payload = GrpcWireByteStringEx.WrapPayload(CacheValueWireCodec.EncodeWireValueToOwned(value, serializer)) },
        };
    }

    internal static CacheEntryWire ToEntryWire<T>(T? value, ISquirixSerializer serializer)
    {
        ArgumentNullException.ThrowIfNull(serializer);

        return new CacheEntryWire
        {
            Payload = GrpcWireByteStringEx.WrapPayload(CacheValueWireCodec.EncodeWireValueToOwned(value, serializer)),
        };
    }

    private static T? Coerce<T>(object? value) => value is T result ? result : default;

    private static T? DecodePayload<T>(ReadOnlySpan<byte> payload, ISquirixSerializer serializer)
    {
        if (!CacheValueWireCodec.TryReadWireValue<T>(payload, serializer, out var value))
    ...
    {
        if (!CacheValueWireCodec.TryReadWireValueUntyped(payload, out var decoded))
            throw new InvalidDataException("Binary cache value payload is invalid or truncated.");
    ...
    private static CacheValue ToCacheValueFromJsonElement(JsonElement json, ISquirixSerializer serializer)
    {
        return json.ValueKind switch
        {
            ...
            _ => new CacheValue { Payload = GrpcWireByteStringEx.WrapPayload(CacheValueWireCodec.EncodeWireValueToOwned(json, serializer)) },
        };
    }
    ...
    private static bool TryReadTypedScalar<T>(CacheValue value, [MaybeNullWhen(false)] out T result)
    {
        switch (value.KindCase)
        {
            ...
            case CacheValue.KindOneofCase.Payload:
                break;
            default:
                throw new InvalidDataException("Unsupported cache value kind.");
        }
