...
internal static partial class BinaryJsonTreeMetadataCodec
{
    ...
    private static int ComputeEncodedLength(object? value, JsonTypeInfo typeInfo)
    {
    ...
    private static int ComputeObjectLength(object value, JsonTypeInfo typeInfo)
    {
        ...
        foreach (var property in typeInfo.Properties)
        {
            ...
            length += 2 + Encoding.UTF8.GetByteCount(property.Name);
...
#pragma warning restore ZA0302
    ...
    private static int Write(object? value, JsonTypeInfo typeInfo, Span<byte> destination)
    {
        ArgumentNullException.ThrowIfNull(typeInfo);
        if (value is null)
