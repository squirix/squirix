...
{
    internal static ListValue ListFromJson(JsonElement el)
    {
    ...
    internal static Value ConvertNumberToProtoValue(JsonElement element)
    {
    ...
    internal static bool TryWriteNumberEnvelope(Utf8JsonWriter writer, Struct s)
    {
        ...
        if (!s.Fields.TryGetValue(ProtoStructCodec.NumberEnvelopeDecimalKey, out var decimalField) || decimalField.KindCase is not Value.KindOneofCase.StringValue || !decimal.TryParse(
