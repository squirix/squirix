...
/// The snapshot array field is volatile, so readers observe it with a single acquire;
/// Volatile.Read(ref ...) is intentionally not used because passing a volatile field
/// by reference voids its volatility (CS0420).
/// <para>
/// This engine intentionally mirrors the server-side metadata chain: the client and server
...
internal static class ClientSerializerMetadata
{
    ...
    private static readonly ContextsSnapshot Contexts = new([SquirixClientJsonContext.Default]);
    ...
    internal static void Serialize(Stream destination, object? value, Type declaredType)
    {
        ...
        // an async state machine per writing would be pure overhead.
