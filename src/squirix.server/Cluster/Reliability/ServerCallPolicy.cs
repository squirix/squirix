...
internal sealed class ServerCallPolicy : IServerCallPolicy
{
    ...
    private void ThrowIfDraining()
    {
        ...
        ServerCallPolicyMetrics.DrainRejectsTotal.WithLabels(_peer).Inc(1);
    ...
    private sealed class ServerCallPolicyExecutor
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            ServerCallPolicyMetrics.QueueWaitSeconds.Observe(_peer, Stopwatch.GetElapsedTime(queueWaitStarted));
            try
            {
                ThrowIfDraining();
        ...
        {
            ServerCallPolicyMetrics.BackoffsTotal.WithLabels(_peer).Inc(1);
            ServerCallPolicyMetrics.BackoffSeconds.Observe(_peer, d);
            ...
            CancellationToken attemptToken)
        {
            ...
            catch (OperationCanceledException oce)
            {
                ...
                ServerCallPolicyMetrics.RetriesTotal.WithLabels(_peer, "operation_canceled").Inc(1);
                                          ...
                                          ServerCancelClassifier.OperationEffectiveTokenAllowsRetryAttempt(effectiveToken))
            {
                ...
                ServerCallPolicyMetrics.RetriesTotal.WithLabels(_peer, rx.StatusCode is StatusCode.DeadlineExceeded ? "deadline_exceeded" : "Canceled").Inc(1);
                                          ...
                                          attempt < _maxAttempts && ServerCancelClassifier.OperationEffectiveTokenAllowsRetryAttempt(effectiveToken))
            {
                ...
                ServerCallPolicyMetrics.RetriesTotal.WithLabels(_peer, ServerCallPolicyRetryClassifier.ClassifyRetryReason(rx)).Inc(1);
                return AttemptOutcome<T>.Retry(await BackoffOrCaptureCancellationAsync(BackoffWithJitter(attempt), rx, effectiveToken).ConfigureAwait(false));
            }
            ...
            {
                ServerCallPolicyMetrics.RetriesTotal.WithLabels(_peer, ServerCallPolicyRetryClassifier.ClassifyRetryReason(ex)).Inc(1);
