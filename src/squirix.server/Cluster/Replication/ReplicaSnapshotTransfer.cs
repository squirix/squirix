    ...
    GroupSnapshot Snapshot)
{
    ...
    internal static ReplicaSnapshotTransfer Create(in GroupSnapshot snapshot, int maxSnapshotBytes = GroupSnapshotStore.DefaultMaxSnapshotBytes)
    {
        ...
        // the encoding pass, and every later IsValidFor read observe identical elements. Records are copied
        // with detached payload buffers so later caller mutations cannot alter checksum-protected content.
    ...
    private static GroupIdempotencyRecord[] CopyOutcomes(IReadOnlyList<GroupIdempotencyRecord> outcomes)
    {
        ...
        for (var i = 0; i < records.Length; i++)
        {
            ...
            {
                OperationFingerprint = record.OperationFingerprint.ToArray(),
                OutcomePayload = record.OutcomePayload.ToArray(),
            };
        }
