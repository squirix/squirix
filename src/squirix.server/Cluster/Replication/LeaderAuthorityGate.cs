...
/// </remarks>
[SuppressMessage("Usage", "MA0182:Internal type is apparently never used", Justification = "Test-only activation seam until failover activation wires the authority gate in a follow-up milestone.")]
internal static class LeaderAuthorityGate
{
    ...
    internal static LeaderAuthorityDecision CheckRead(
        int replicaCount,
        ...
        LeaderReadState read)
    {
        ...
        if (replicaCount > 1 && read.AppliedIndex < read.ReadIndex)
            return new LeaderAuthorityDecision(false, LeaderAuthorityDenial.ReadIndexNotApplied, FollowerLogRefusal.NotReady);

        return new LeaderAuthorityDecision(true, LeaderAuthorityDenial.None, string.Empty);
    }
    ...
    internal static LeaderAuthorityDecision CheckWrite(
        int replicaCount,
        bool hasMajorityContact,
        bool isLeader,
        ulong currentTerm,
        ulong observedTerm)
    {
        ...
        if (observedTerm > currentTerm)
            return new LeaderAuthorityDecision(false, LeaderAuthorityDenial.StaleTerm, FollowerLogRefusal.StaleTerm);

        if (!isLeader)
            return new LeaderAuthorityDecision(false, LeaderAuthorityDenial.NotLeader, FollowerLogRefusal.NotReady);

        // The minority fails closed: without majority contact neither reads nor writes are served.
        if (!hasMajorityContact)
            return new LeaderAuthorityDecision(false, LeaderAuthorityDenial.MinorityFenced, FollowerLogRefusal.NotReady);
