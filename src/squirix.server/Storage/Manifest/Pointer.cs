...
internal static class Pointer
{
    ...
    internal static bool IsValidPointer(ReadOnlySpan<byte> source) => source.Length >= Magic.Length && source.StartsWith(Magic);

    internal static int Read(ReadOnlySpan<byte> source)
    {
