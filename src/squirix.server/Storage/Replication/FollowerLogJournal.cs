...
/// </remarks>
[Immutable]
internal sealed class FollowerLogJournal
{
    ...
    internal ReadOnlySortedDictionary<ulong, FollowerLogEntry> Entries => new(_entries);

    internal ReadOnlySortedDictionary<ulong, (long Offset, ulong Term)> EntryOffsets => new(_entryOffsets);

    internal FollowerLogPaths Paths { get; }

    internal IFollowerLogSnapshotStore Snapshot { get; }
    ...
    internal void AddEntryOffset(ulong index, long offset, ulong term) => _entryOffsets[index] = (offset, term);

    internal void AdvanceBaseline(SnapshotBaseline baseline)
    {
    ...
    {
        // Caller hold _gate; both indexes reset together before a rebuild repopulates them.
        _entries.Clear();
        _entryOffsets.Clear();
    }
    ...
    internal void ReleaseAppliedEntries(ulong index)
    {
    ...
    internal void RemoveEntriesAbove(ulong index) => RemoveEntriesAboveCore(index);
