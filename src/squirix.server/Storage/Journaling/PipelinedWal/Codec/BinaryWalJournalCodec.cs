...
/// <summary>Binary frame body codec for PipelinedWal (SJRN v2).</summary>
internal sealed class BinaryWalJournalCodec : IJournalFrameCodec
{
    private const int FixedPrefixSize = 8 + 8 + 1 + 2 + 2 + 2 + 4;

    public static BinaryWalJournalCodec Instance { get; } = new();

    public byte FileVersion => WalBinaryFraming.FileVersion;
    ...
    public JournalRecord Decode(ReadOnlySpan<byte> frameBody)
    {
        ...
        {
            WalOpcode.Put => new JournalRecord
            {
                Sequence = seq,
                UnixMs = unixMs,
            ...
            WalOpcode.Remove => new JournalRecord
            {
                Sequence = seq,
                UnixMs = unixMs,
            ...
            WalOpcode.RemoveExpiration => new JournalRecord
            {
                Sequence = seq,
                UnixMs = unixMs,
            ...
            WalOpcode.TouchExpiration => new JournalRecord
            {
                Sequence = seq,
                UnixMs = unixMs,
    ...
    public int Encode(JournalRecord record, Span<byte> destination)
    {
        ...
        destination[16] = WalOpcodeWire.ToWireValue(ToOpcode(record.Operation));
    ...
    private static WalOpcode ToOpcode(JournalOperationKind operation) => operation switch
    {
        JournalOperationKind.Put => WalOpcode.Put,
        JournalOperationKind.Remove => WalOpcode.Remove,
        JournalOperationKind.RemoveExpiration => WalOpcode.RemoveExpiration,
        JournalOperationKind.TouchExpiration => WalOpcode.TouchExpiration,
