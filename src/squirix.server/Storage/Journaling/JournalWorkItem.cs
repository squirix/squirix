...
{
    private JournalWorkItem(JournalWorkKind kind, PooledAck? ack, byte[]? frameBytes, int frameLength, int resetSegmentIndex, ulong resetSequence)
    {
        Kind = kind;
        Ack = ack;
    ...
    internal static JournalWorkItem Append(byte[] frameBytes, int frameLength, PooledAck? ack = null) => new(JournalWorkKind.Append, ack, frameBytes, frameLength, 0, 0UL);
    ...
    internal static JournalWorkItem AppendWithDurability(PooledAck ack, byte[] frameBytes, int frameLength)
    {
    ...
    internal static JournalWorkItem DurabilityCheckpoint(PooledAck ack)
    {
        ArgumentNullException.ThrowIfNull(ack);
    ...
    internal static JournalWorkItem MaintenanceBegin(PooledAck ack)
    {
        ArgumentNullException.ThrowIfNull(ack);
    ...
    internal static JournalWorkItem MaintenanceEnd(PooledAck ack, int resetSegmentIndex, ulong resetSequence)
    {
        ArgumentNullException.ThrowIfNull(ack);
