...
internal sealed class DomainErrorMappingCacheDecorator<T> : ILogicalNamespacedCache<T>
{
    ...
    public ValueTask InsertAsync(string cacheName, string key, T? value, CancellationToken cancellationToken) =>
        WithMappingAsync(ct => _inner.InsertAsync(cacheName, key, value, ct), cancellationToken);

    public ValueTask InsertAsync(string cacheName, string key, CacheEntry<T> entry, CancellationToken cancellationToken) =>
        WithMappingAsync(ct => _inner.InsertAsync(cacheName, key, entry, ct), cancellationToken);
