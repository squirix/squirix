...
internal sealed class LocalOwnerPutPayloadSizeGuardCacheDecorator<T> : ILogicalNamespacedCache<T>
{
    private readonly ILogicalNamespacedCache<T> _inner;
    private readonly INodeLocator _ring;
    private readonly string _self;

    public LocalOwnerPutPayloadSizeGuardCacheDecorator(string self, INodeLocator ring, ILogicalNamespacedCache<T> inner)
    {
    ...
    public async ValueTask<bool> UpdateAsync(string operationId, string cacheName, string key, T? value, CancellationToken cancellationToken)
    {
        ...
        EntryPayloadSizeGuard.EnsureEncodedLengthWithinLimit(
            new CacheEntry<T>
            {
                Value = value,
