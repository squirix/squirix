...
internal sealed class BootstrapManifestStore
{
    ...
    private static int ComputeEncodedLength(BootstrapManifest manifest)
    {
        ...
        return length > maxPayload ? throw new InvalidOperationException("Bootstrap manifest exceeds the maximum size.") : int.CreateChecked(length);
    }
    ...
    private static BootstrapManifest Decode(byte[] bytes, int offset, int length)
    {
        try
        {
            ...
            return stream.Position != stream.Length ? throw new InvalidDataException("Bootstrap manifest contains trailing bytes.") : new BootstrapManifest
            {
    ...
    private static BootstrapManifest DecodeFile(byte[] bytes)
    {
        ...
        var calculatedChecksum = Crc32C.Compute(bytes.AsSpan(HeaderLength, payloadLength));
        return calculatedChecksum != expectedChecksum ? throw new InvalidDataException("Bootstrap manifest checksum is invalid.") : Decode(bytes, HeaderLength, payloadLength);
    }
    ...
    private static byte[] Encode(BootstrapManifest manifest)
    {
        ...
        return stream.Length != payloadLength ? throw new InvalidOperationException("Bootstrap manifest changed during encoding.") : stream.ToArray();
    }
    ...
    private static byte[] ReadBytes(BinaryReader reader, int requiredLength)
    {
        ...
        return bytes.Length != length ? throw new EndOfStreamException() : bytes;
    }
    ...
    private static string ReadString(BinaryReader reader)
    {
        ...
        var bytes = reader.ReadBytes(length);
        return bytes.Length != length ? throw new EndOfStreamException() : Encoding.UTF8.GetString(bytes);
    }
    ...
    private static int Utf8ByteCount(string value, string field)
    {
        ...
        return bytes > MaximumStringBytes ? throw new InvalidOperationException($"Bootstrap manifest {field} exceeds the maximum length.") : bytes;
    }
