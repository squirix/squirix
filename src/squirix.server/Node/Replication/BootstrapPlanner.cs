...
internal sealed class BootstrapPlanner
{
    ...
    private static BootstrapManifest CreateManifest(BootstrapPreparationRequest request)
    {
    ...
    private static bool MatchesAttempt(BootstrapManifest existing, BootstrapManifest candidate)
    {
    ...
    private static bool NullableUriEquals(Uri? source, Uri? target) => source == null ? target == null : target != null && UriEquals(source, target);

    private static bool UriEquals(Uri source, Uri target) => string.Equals(source.AbsoluteUri, target.AbsoluteUri, StringComparison.Ordinal);
