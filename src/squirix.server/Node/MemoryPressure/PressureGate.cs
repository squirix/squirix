...
internal sealed class PressureGate : IMemoryPressureGate
{
    ...
    public void ThrowIfMemoryGrowingWriteRejected(long estimatedNetGrowthBytes, bool magnitudeUnknown, string operation)
    {
        ...
        var notCritical = _evaluator.Evaluate(currentBytes) is not PressureLevel.Critical;
        if (notCritical && (magnitudeUnknown || boundedGrowth <= 0 || _evaluator.Evaluate(AddSaturating(currentBytes, boundedGrowth)) is not PressureLevel.Critical))
            return;
