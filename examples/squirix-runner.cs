...
static async Task RunDemoLoadAsync(ICache<object?> cache, CancellationToken cancellationToken)
{
    ...
    while (!cancellationToken.IsCancellationRequested)
    {
        try
        {
            ...
            {
                // Plain-string payload: strings are covered by the built-in source-generated
                // metadata, so the load needs no registered context (anonymous types would
                // throw InvalidOperationException from the AOT-safe metadata resolver instead).
