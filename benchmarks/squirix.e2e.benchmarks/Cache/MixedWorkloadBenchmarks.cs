...
public sealed class CacheMixedWorkloadBenchmarks : CacheBenchmarkBase
{
    ...
    /// <summary>Measures a 95 percent read, 5 percent write workload.</summary>
    /// <returns>A task that completes when the batch has finished.</returns>
    [Benchmark(OperationsPerInvoke = BatchSize)]
    ...
    /// <summary>Measures an 80 percent read, 15 percent write, 5 percent remove workload.</summary>
    /// <returns>A task that completes when the batch has finished.</returns>
    [Benchmark(OperationsPerInvoke = BatchSize)]
    ...
    /// <summary>Measures a read-mostly workload against remote-owner key selection when the scenario uses it.</summary>
    /// <returns>A task that completes when the batch has finished.</returns>
    [Benchmark(OperationsPerInvoke = BatchSize)]
    ...
    /// <summary>Measures a read-mostly workload over the scenario key distribution.</summary>
    /// <returns>A task that completes when the batch has finished.</returns>
    [Benchmark(OperationsPerInvoke = BatchSize)]
