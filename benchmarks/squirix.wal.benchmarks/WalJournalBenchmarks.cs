...
/// <summary>Compares JsonFramed vs PipelinedWal journal append throughput.</summary>
[MemoryDiagnoser]
...
public class WalJournalBenchmarks
{
    ...
    /// <summary>Gets or sets the platform segment writer for pipelined WAL.</summary>
    [Params(WalPlatformBackend.RandomAccess)]
    [SuppressMessage("ReSharper", "UnusedAutoPropertyAccessor.Global", Justification = "Property annotated with [Params] must have a public setter")]
    public WalPlatformBackend PlatformBackend { get; set; }
    ...
    {
        var dir = DirectoryKit.CreateTempDirectory("wal-bench");
        var options = new PersistenceOptions
        {
            DataDir = dir,
            JournalBackend = Backend,
            WalPlatformBackend = PlatformBackend,
            JournalGroupCommitMaxWaitMs = GroupCommitMaxWaitMs,
            JournalGroupCommitMaxBatch = 32,
            JournalMaxSegmentMb = 64,
        };
        _host = await WalBenchmarkHost.CreateAsync(options, CancellationToken.None).ConfigureAwait(false);
    ...
    public async Task AppendPutAsync()
    {
        ...
        {
            await host.Coordinator.AppendPutAsync(_key, _putPayload, operationId: null, CancellationToken.None).ConfigureAwait(false);
