...
| version | `i64` |
| tagCount | `u16` |
| tags | repeated: `u16` key length, `u16` value length, key UTF-8, value UTF-8 |
| valueKind | `u8` |
...
| JsonBlob | 7 | `u32` length + raw UTF-8 JSON |

Complex CLR types are serialized to UTF-8 JSON under `JsonBlob` on write and deserialized on read.
...
Writes go to `sn-NNNNNN.tmp` and are published atomically to `sn-NNNNNN.bsqx` via `IStorageFileOperations.PublishSnapshot`.
