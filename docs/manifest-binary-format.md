...
Publish order: write the numbered `.bmqx` file and flush it to disk, then overwrite `man-current` in place
(write-through on Windows). A crash after the data file but before the pointer update leaves an orphan numbered file;
recovery continues to use the previous pointer.
